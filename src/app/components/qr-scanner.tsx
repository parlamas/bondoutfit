...
// The correct import is 'Scanner' not 'QrScanner'
const Scanner = dynamic(
  () => import('@yudiel/react-qr-scanner').then((mod) => mod.Scanner),
  {
    ssr: false,
    loading: () => <div className="w-[300px] h-[300px] bg-gray-200 animate-pulse rounded-lg"></div>
  }
);
...
export default function QRScanner({ onScan, onError, className = '' }: QRScannerProps) {
  ...
  const handleScan = (result: any) => {
    ...
    setScanMessage(`Error: ${error instanceof Error ? error.message : String(error)}`);
    onError?.(error);
  ...
  if (!hasCamera) {
    return (
      <div className={`p-6 bg-red-50 border border-red-200 rounded-xl ${className}`}>
        <h3 className="font-semibold text-red-800 mb-2">Camera Not Available</h3>
        <p className="text-red-600 text-sm mb-4">
          Your device doesn't have a camera or camera access is blocked.
        </p>
        <div className="text-sm text-gray-700 space-y-1">
          <p>• Check if your device has a camera</p>
