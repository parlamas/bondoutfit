//src/components/PsychologyLanguageSwitcher.tsx
...
export default function PsychologyLanguageSwitcher({
  value,
  onChange,
}: {
  value: string;
  ...
  function select(code: string) {
    document.cookie = `psychLocale=${code};path=/;max-age=${60 * 60 * 24 * 365}`;
    onChange(code);
    setOpen(false);
  }
        ...
        <div className="absolute right-0 mt-1 w-56 max-h-80 overflow-y-auto bg-white border border-brand-stoneborder rounded-md shadow-lg z-50">
          {LANGUAGES.map(l => (
            <button
              ...
              className={`flex items-center gap-2 w-full text-left px-3 py-2 text-sm hover:bg-brand-paper ${l.code === value ? 'bg-brand-paper font-semibold' : ''}`}
            >
